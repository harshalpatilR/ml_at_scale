...
flight_df <- flight_df %>% na.omit() %>% head(1000)
...
sdf_schema(flight_df_mutated) %>% as.data.frame %>% t
...
#    input_cols = c("ORIGIN_INDEXED","OP_CARRIER_INDEXED","DEST_INDEXED"),
#    output_cols = c("ORIGIN_ENCODED","OP_CARRIER_ENCODED","DEST_ENCODED")
#  ) %>%
  ft_r_formula(
    CANCELLED ~ 
    CRS_ELAPSED_TIME +
